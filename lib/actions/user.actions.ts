...
import { CustomError, ShippingAddress } from "@/types";
import { z } from "zod";
...
export async function signUpUser(prevState: unknown, formData: FormData) {
  try {
    ...
    return { success: true, message: "Signed registered successfully" };
  } catch (error) {
    if (isRedirectError(error)) {
      throw error;
    }
...
export async function deleteUser(
  ...
  try {
    ...
    return {
      success: false,
      message: formatErrors(error as CustomError) || "Failed to delete user",
    };
  }
}
...
export async function updateUser(user: z.infer<typeof updateUserSchema>) {
  try {
    ...
    return {
      success: false,
      message: formatErrors(error as CustomError) || "Failed to update user",
    };
  }
}
